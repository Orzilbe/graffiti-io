...
const io = new Server(httpServer, {
    ...
    // ── Latency tuning ──────────────────────────────────────────────────────
    // Skip the HTTP long-poll upgrade handshake — connect straight to WS.
    // This alone removes 1-2 round trips on first connect.
    transports: ['websocket'],
    // Nagle's algorithm buffers small packets to batch them — terrible for games.
    // perMessageDeflate adds CPU overhead and latency; turn it off.
...
// ── Delta tracking ────────────────────────────────────────────────────────
// Instead of sending 12 000 cells every tick, we track which indices changed
// this tick and only send those.  The client applies the patch on top of its
// own copy of the grids, so state stays in sync with far less bandwidth.
let prevTerritory = [];   // snapshot from end of last tick
let prevTrail     = [];   // snapshot from end of last tick
let lbTick        = 0;    // throttle leaderboard to ~5 fps (every 4 ticks)
...
function gameTick() {
    ...
    // ── Build grid deltas ──────────────────────────────────────────────────
    // Collect only the cells that changed since the previous tick.
    // Format: flat array of [index, newValue, index, newValue, ...]
    // This is far more compact than sending all 6 000 cells every 50 ms.
    const terrDelta  = [];
    const trailDelta = [];
    ...
    io.to([...displays]).emit('game-state', {
        ...
        terrDelta,    // only changed territory cells
        trailDelta,   // only changed trail cells
    });

    // ── Leaderboard at ~5 fps (every 4 ticks = 200 ms) ────────────────────
    // No need to push ranking 20× per second — the display animation is 400 ms.
    if (++lbTick % 4 === 0) {
...
function startGame() {
    ...
    calcTerritory();
    // Initialise delta snapshots so the first tick doesn't diff against stale data
    prevTerritory = territory.slice();
    prevTrail     = trailGrid.slice();
    lbTick        = 0;
...
io.on('connection', socket => {
    ...
    socket.on('display-join', () => {
        ...
        // Send the FULL grid on first connect — this is the client's baseline.
