...
// ── State ──────────────────────────────────────────────────────────────────
// gs holds all mutable game state.  The grids are our LOCAL copy — the server
// sends only what changed each tick and we apply the patch here.
...
// ── Full state — received once on connect (and on reconnect) ──────────────
// This seeds our local grid copy.  All subsequent ticks use deltas only.
...
// ── Delta tick — received every 50 ms during a game ───────────────────────
// terrDelta / trailDelta are flat arrays: [idx, val, idx, val, ...]
// We apply them in-place — no allocation, no full copy.
socket.on('game-state', s => {
    // Check for deaths before we clobber player state
    checkDeaths(gs, s);

    // Apply territory delta
    ...
    // Update the rest of the state (players, timer, running flag)
    gs.running  = s.running;
    gs.timeLeft = s.timeLeft;
    gs.players  = s.players;

    // Recompute drips only when territory actually changed
...
function checkDeaths(prev, curr) {
    ...
    for (const p of curr.players) {
        ...
        if (was?.alive && !p.alive) spawnSplatter(was.x, was.y, COLORS[p.id]);
    }
}
